# Apace — Project Conventions

## What is this?
macOS menu bar dictation app (product name **Apace**, domain apace.so) — *type at the speed of thought*. Hold a hotkey, speak, release — text appears in any app. Powered by WhisperKit (on-device, CoreML + Neural Engine), with a real-time Apple `SpeechAnalyzer` streaming path on macOS 26+.

> **Naming:** product, target, Swift module, and bundle are all **Apace** (`PRODUCT_NAME` / `PRODUCT_MODULE_NAME = Apace`, bundle id `so.apace`, repo `Lyons800/apace`). `import Apace`, `ApaceApp`, `ApaceConfig`, the `Apace/` source dir, `ApaceTests`.
...
- Config struct: `ApaceConfig` (type name unchanged; stored as `apace_config` in UserDefaults)
- NSLog prefix: `[Apace]`
- App Support path: `~/Library/Application Support/Apace/`

## File Layout
- `Apace/Core/` — business logic (audio, transcription, hotkeys, text insertion)
- `Apace/UI/` — SwiftUI views and AppKit controllers
- `Apace/Models/` — state, config, result types
- `Apace/Utilities/` — permissions, key codes, sounds
- `Apace/Resources/` — sound effects
...
2. Add the `MLXLLM` and `MLXLMCommon` products to the Apace target
