# Apace — Launch Runbook

Status as of 2026-05-31. This is the checklist to take Apace from "feature-complete on my Mac" to "downloadable, signed, auto-updating public app."
...
- App is feature-complete (`Apace.app`, bundle `dev.apace.app`, repo `Lyons800/apace` — public).
...
- `homebrew-apace/Casks/apace.rb` — Homebrew cask (secondary channel).
- Landing site (`../apace-site`) is live at **apace.dev**.
- **Fixed 2026-05-31:** every `oisinlyons/apace` URL corrected to `Lyons800/apace` — this includes the app's Sparkle feed URL (`INFOPLIST_KEY_SUFeedURL`, both build configs), the cask, and all 5 site links. A shipped app would otherwise have checked for updates at a dead URL.
...
1. https://account.apple.com → Sign-In and Security → **App-Specific Passwords** → generate one, label it "apace-notary".
...
```bash
cd ~/Projects/GitHub/Active/Apace
sed -i '' 's/BWD692VD35/MDY7H22G36/g' Apace.xcodeproj/project.pbxproj
grep -c MDY7H22G36 Apace.xcodeproj/project.pbxproj   # expect 8
```
Also confirm the GitHub secret `APPLE_TEAM_ID` (already set) equals the same value:
```bash
# can't read secret values; re-set to be sure:
gh secret set APPLE_TEAM_ID -R Lyons800/apace --body "MDY7H22G36"
```
...
gh secret set APPLE_CERTIFICATE_P12 -R Lyons800/apace < <(base64 -i ~/path/to/certificate.p12)
gh secret set APPLE_CERTIFICATE_PASSWORD -R Lyons800/apace --body "<p12 export password>"
gh secret set APPLE_ID -R Lyons800/apace --body "oisinlyons13@gmail.com"
gh secret set APPLE_APP_PASSWORD -R Lyons800/apace --body "<app-specific password from §2>"

gh secret list -R Lyons800/apace   # expect 6 secrets total
```
...
```
The tag push triggers `.github/workflows/release.yml`, which: builds → signs (Developer ID) → creates DMG → **notarizes + staples** → Sparkle-signs → generates `appcast.xml` → publishes a GitHub Release with `Apace.dmg` + `appcast.xml` attached.

Watch it: `gh run watch -R Lyons800/apace`

---

## §6 — Verify the launch

- **Download:** the site's "Download Free" buttons point to `github.com/Lyons800/apace/releases/latest` — confirm it resolves to the DMG once the release exists.
- **Gatekeeper:** on a *different* Mac (or after removing your dev signing), download the DMG and open `Apace.app` — it should open with **no** "unidentified developer" warning. If it warns, notarization didn't staple.
  - Check: `spctl -a -t open --context context:primary-signature -v ~/Downloads/Apace.dmg`
  - Check: `xcrun stapler validate ~/Downloads/Apace.dmg`
...
- **Homebrew (optional):** the cask still has `sha256 "REPLACE_WITH_DMG_SHA256"`. After the release, set it:
  `shasum -a 256 Apace.dmg` → paste into `homebrew-apace/Casks/apace.rb`, bump `version` to `1.0.0`, and publish the tap as repo `Lyons800/homebrew-apace` so users can `brew install --cask lyons800/apace/apace`.
