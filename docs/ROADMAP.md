# Murmur — Product & Monetization Roadmap

_Last updated: 2026-05-31_

## 1. Positioning (the one-liner)

**Murmur transcribes for free. Murmur Pro is your voice-powered AI — it edits, learns, commands, and controls your Mac.**

The competitive research (2026) is unambiguous: raw transcription is **commoditized** (Parakeet is CC-BY, WhisperKit MIT, Apple SpeechAnalyzer free, Ollama cleanup free), and free open-source rivals already replicate Murmur's exact engine stack (macparakeet, Handy ~22.8k★, FluidVoice). Meanwhile the category is moving **dictation → on-device AI editing → voice agent**. So Murmur's defensible value lives **up the stack**, not in the engine.
...
- Cloud-heavy features (screen reasoning in Command Mode) use **BYOK** (user's own Claude/OpenAI key) so Murmur never eats per-use cloud costs and the one-time model stays sustainable.
- Optional later: a managed-cloud credits/subscription add-on for non-technical users who won't bring an API key. Not v1.

### Free vs Pro split

| | **Murmur (Free, open-source)** | **Murmur Pro (one-time license)** |
|---|---|---|
...
### Pricing (proposal)
- **Murmur Pro: $39 one-time** (lifetime, 2 Macs). Anchors above VoiceInk ($25) and below MacWhisper (€59); undercuts every subscription's first-year cost ($84–180/yr). BYOK for Command Mode.
...
Murmur's version, on-brand (privacy-first, on-device-where-possible + BYOK):
...
This is the feature that turns Murmur from "a dictation app" into "a voice interface to your Mac" — the differentiator vs every transcription competitor.
...
- Resolve brand collision (multiple apps named "Murmur") + pick the real domain.
- Ship the landing-page overhaul reflecting open-core + Pro.

**Phase 1 — Open-core foundation (weeks 2–4)**
- **Open-source the free core** (license: MIT or GPLv3 — decide; GPLv3 protects against closed forks, MIT maximizes adoption). Split the repo: open `murmur-core` (transcription) vs closed Pro module, OR single repo with Pro features behind a license gate.
...
- **Brand collision** — multiple "Murmur" apps muddy search. Resolve domain + naming early.
