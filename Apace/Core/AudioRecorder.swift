...
final class AudioRecorder {
    ...
    private let bufferQueue = DispatchQueue(label: "audio.apace.sobuffer")
    private var levelCallback: ((Float) -> Void)?
    /// Optional live feed of freshly-captured 16 kHz mono float chunks, delivered
    /// on the main queue (drives a real-time streaming transcriber).
    ...
    func warmUp() throws {
        ...
        NSLog("[Apace] AudioRecorder: warmed up, input format=\(inputFormat), sampleRate=\(inputFormat.sampleRate)")
    }

    func startRecording(levelCallback: ((Float) -> Void)? = nil,
                        sampleCallback: (([Float]) -> Void)? = nil) throws {
