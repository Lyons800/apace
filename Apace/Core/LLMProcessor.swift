...
final class LLMProcessor {
    ...
    func loadModel(progressCallback: ((Double) -> Void)? = nil) async throws {
        ...
        self.isLoaded = true
        NSLog("[Apace] LLM model loaded: \(Self.defaultModelID)")
        #else
        NSLog("[Apace] LLM not available — add mlx-swift-lm SPM package to enable")
        #endif
    }
    ...
    func process(text: String, context: AppContext, command: VoiceCommand? = nil) async -> String {
        ...
        do {
            ...
            NSLog("[Apace] LLM cleaned: '\(text)' → '\(cleaned)'")
            ...
            if cleaned.isEmpty || cleaned.count > text.count * 2 {
                NSLog("[Apace] LLM output rejected (empty or >2x length), using original")
                return text
            }

            return cleaned
        } catch {
            NSLog("[Apace] LLM processing failed: \(error.localizedDescription)")
            return text
        }
        #else
        return text
    ...
    func rewrite(_ text: String, instruction: String) async -> String {
        ...
        do {
            ...
            NSLog("[Apace] Voice-edit: '\(instruction)' → '\(edited.prefix(60))'")
            return edited.isEmpty ? text : edited
        } catch {
            NSLog("[Apace] Voice-edit rewrite failed: \(error.localizedDescription)")
            return text
        }
        #else
        return text
