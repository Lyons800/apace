...
struct SettingsView: View {
        ...
        Form {
            Section("Command Mode") {
                ...
                case .byok:
                    ...
                    Text("Stored in your macOS Keychain. Used only to call Claude directly from your Mac — Apace never sees it.")
                        .font(.caption).foregroundStyle(.secondary)
                case .managed:
                    Text("Apace Pro managed cloud is coming soon — use your own API key for now.")
                        .font(.caption).foregroundStyle(.secondary)
                case .local:
            ...
            Section {
                Text("Hold ⇧ + your dictation key, then speak: “turn the volume down”, “add this to my calendar”, “what does this error mean?”. Apace sees your screen and acts. Risky actions ask before running.")
                    .font(.caption).foregroundStyle(.secondary)
            }
        }
        .padding()
    ...
    private var textTab: some View {
        Form {
            ...
            Section("Context Detection") {
                Text("Apace detects the active app and adjusts formatting — code editors preserve casing, chat apps remove trailing periods, emails add them.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
    ...
    private var dictionaryTab: some View {
        Form {
            Section {
                Text("Add words and their correct spellings. Apace will replace spoken forms with the correct version after transcription and hint the speech model for better recognition.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
    ...
    private func setLaunchAtLogin(_ enabled: Bool) {
        do {
            ...
            NSLog("[Apace] Launch at login: \(enabled)")
        } catch {
            NSLog("[Apace] Failed to set launch at login: \(error.localizedDescription)")
        }
    }
}
