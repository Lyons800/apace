...
struct ApaceConfig: Codable {
    ...
    static let `default` = ApaceConfig()

    private static let storageKey = "apace_config"

    static func load() -> ApaceConfig {
        if let data = UserDefaults.standard.data(forKey: storageKey),
           let config = try? JSONDecoder().decode(ApaceConfig.self, from: data) {
            return config
        }
        return .default
    }
    ...
    func save() {
        if let data = try? JSONEncoder().encode(self) {
            UserDefaults.standard.set(data, forKey: ApaceConfig.storageKey)
        }
    }
}
...
// breaking callers such as `ApaceConfig()` and `ApaceConfig.default`.
